...
def get_month_and_label(date_string):
    """
    Given an ISO-formatted date string (YYYY-MM-DD HH:MM:SS), return
    (month, month_label):
    - month: sortable key, 'YYYY-MM' (e.g. '2025-07')
    - month_label: friendly display string, 'Mon YYYY' (e.g. 'Jul 2025')

    Based on the chat's actual date (conversation_start_time), NOT
    week_ending - a week spanning a month boundary can therefore contain
    chats attributed to two different months, which is intentional and
    required for accurate month-level totals.
    """
    if not date_string or date_string == 'nan' or str(date_string).strip() == '':
        return '', ''

    if str(date_string).startswith('DATE_'):
        return 'UNPARSED_DATE', 'UNPARSED_DATE'
    ...
    try:
        ...
        print(f"Warning: Could not calculate month for: {date_string}")
        return 'INVALID_DATE', 'INVALID_DATE'
...
def clean_attribute_value(value):
    """
    Strip whitespace and wrapping quote characters from a generic
    participant-attribute value (e.g. accountType). Same quote-stripping
    behavior as clean_last_page_value, but without the lastPage-specific
    'mesh' PII-redaction fix, which doesn't apply to other attributes.
    """
    value = str(value).strip()
    while len(value) >= 2 and value[0] in '"\'' and value[-1] == value[0]:
        value = value[1:-1].strip()
...
def process_chat_data(input_file, output_file, json_file=None, lastpage_lookup=None, all_conv_ids=None):
    ...
    for index, row in df.iterrows():
        ...
        # Calculate month / month_label for month-on-month PowerBI grouping,
        # based on the chat's actual date (not week_ending)
        month, month_label = get_month_and_label(conversation_start_time)
        ...
        # Read participant attributes once - accountType is pulled from this
        # for every chat, while lastPage/returnCount remain Tobi-only below.
        participant_attributes = str(row.get('Participant Attributes Formatted', '')).strip()
        has_participant_attributes = (
            participant_attributes and participant_attributes != 'nan' and participant_attributes != ''
        )

        # Extract accountType value (all chats). Blank if the key isn't
        # present, and blank if there are no participant attributes at all.
        account_type = ''
        if has_participant_attributes:
            account_type_match = re.search(r'accountType:([^;]+)', participant_attributes)
            if account_type_match:
                account_type = clean_attribute_value(account_type_match.group(1))

        # Extract lastPage/returnCount values (only for Tobi chats)
        last_page = ''
    ...
    # ------------------------------------------------------------------
    dimension_columns = [
        'week_ending', 'month', 'month_label', 'is_non_interactive', 'is_tobi_chat', 'is_contained', 'is_escalated',
        'account_type', 'queue_name', 'last_page', 'last_page_final', 'return_count', 'is_return_chat'
    ]
    ...
    # Reorder columns: time dimensions up front, then conversation_start_time,
    # chat_count, then the rest of the dimensions
    # (return_count_numeric sits right after return_count)
    time_dims = ['week_ending', 'month', 'month_label']
    dims_with_numeric = []
    ...
    column_order = time_dims + ['conversation_start_time', 'chat_count'] + dims_with_numeric
    ...
    try:
        ...
        bool_columns = ['is_tobi_chat', 'is_contained', 'is_escalated', 
                       'is_non_interactive', 'is_return_chat']
        ...
        print(f"Chats with accountType: {aggregated_df[aggregated_df['account_type'] != '']['chat_count'].sum()}")
        ...
        # Show unique months (for month-on-month reporting)
        unique_months = sorted(aggregated_df['month'].unique())
        print(f"\n--- Months in Dataset ({len(unique_months)} months) ---")
        for m in unique_months:
            month_rows = aggregated_df[aggregated_df['month'] == m]
            count = month_rows['chat_count'].sum()
            rows = len(month_rows)
            label = month_rows['month_label'].iloc[0] if rows > 0 else m
            print(f"{label} ({m}): {count} chats ({rows} aggregated rows)")
